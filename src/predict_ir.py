...
class Inpainter:
    def __init__(self, weights_path: str, config_path: str):
        net_onnx = ie.read_network(weights=weights_path, model=config_path)
...
def get_args():
    ...
    arg("-c", "--config", type=str, help="Path to config.", required=True)
...
def main():
    ...
    model = Inpainter(weights_path=args.weights, config_path=args.config)
    ...
    cv2.imwrite("result_ir.jpg", result)
